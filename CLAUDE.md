...
- **lib/ball-shared.ts** — 共有ロジック (React 非依存、定数とグリッド基本のみ)。定数 (`CELL_SIZE`, `BALL_RADIUS`, `COLOR_PRESETS`, カメラプリセット, `NFC_DIRECTIONS`, `NFC_ICONS`, `MAX_LOOP_REPEAT`)、`PatternConfig` インターフェース、`makeShaderMaterial()`、`moveGrid(gridSize, obstacles?)`、`generateRandomPath(gridSize, obstacles?)`、`isHorizontalDir()`。
- **lib/program.ts** — プログラム中間表現 (React 非依存)。トークン解析 (`findMatchingSlash()`, `splitPBlock()`)、エンコード/デコード (`encodeProgram()`, `decodeProgram()`)、ループ展開 (`expandProgram()`, `expandProgramWithMap()`: BRANCH ブロック内トークンを元ソースの絶対インデックスにマップ)、表示グルーピング (`groupProgramForDisplay(program, baseIndex?)`, `displayStepsToFlat()`)、`PBlock` / `DisplayStep` インターフェース。ball-shared から分離。
- **lib/levels.ts** — レベル定義 (React 非依存)。`LevelConfig` インターフェース (`obstacleCount?`, `branchCount?`, `hasChallenge` 含む)、`BranchCell` インターフェース、`LEVELS` レジストリ (lv1, lv2, lv3)、`generateStartGoal()`、`generateLevel()` (スタート/ゴール/障害物/分岐セル一括生成)、`generateObstacles()` + `hasPath()` (BFS 経路検証)、`generateBranchCells()` (中央3x3内限定、デッドロック回避)、`generateChallengeCount()`、`gridCenter()`、`checkMoveResult(branchUsed)` / `checkProgramResult(branchUsed)`、`resolveBranchDir()`、`buildLevelNtagParams()`、`encodeObstacles()` / `decodeObstacles()`、`encodeBranchCells()` / `decodeBranchCells()`。新レベル追加時はここに `LevelConfig` エントリを追加する。
- **lib/useDemoMode.ts** — Playground の自動デモ (attract mode) フック。ボールをランダムに動かしつつ模様も変化させる。`levels.GridPos` を共通利用。
...
- **lib/useNfcScanner.ts** — NFC スキャナフック (React フック)。`/api/nfc/read` を 400ms ポーリングして `nfcConnected`, `nfcFlash` を提供、`onCard(cardId)` コールバックで各カードイベントを通知。`setFlash(cardId, durationMs)` でフラッシュ UI の表示時間を呼び出し側が制御。onCard コールバックは内部 ref でラップされ stale closure を回避。
- **app/components/icons.tsx** — 共有 SVG アイコンモジュール。`IconBall`, `IconGamepad`, `IconListOrdered`, `IconRepeat`, `IconGitBranch`, `IconKeyboard`, `IconCard`, `IconCode`, `IconArrowLeft`, `IconSparkles`, `IconTarget`, `IconLightbulb`, `IconCheck`, `IconX`, `IconRefresh`, `IconSpinner`, `IconPlay`, `IconSettings`, `IconSave`, `IconCardReader`, `IconChevron`。Ball.tsx / help ページ / NfcWriter / モーダル / パネルから参照。
- **app/components/NtagWriteModal.tsx** — NTAG 書き込みモーダル。4 状態 (initial / writing / success / error) を props `writing`, `result` で切替。ソナーアニメーションを内蔵。
- **app/components/SettingsPanel.tsx** — 設定パネル (右上)。開閉トグル、2D/3D 切替、パターン選択 (checker/stripe)、色プリセット、スケールスライダ、ヘルプリンク。`patternConfig`, `setPatternConfig` を props 経由で受ける。
- **app/components/ProgrammingPanel.tsx** — プログラミングパネル (左上)。ヘッダー開閉 + New/ステップリスト/Run ボタン + P-block エディタを含む。内部で `ProgramStepRow` (ドラッグ並び替え + 削除)、`PBlockEditor`, `BranchHeader`, `BranchSteps` に分解し if/else 重複を除去。drag 状態 (`dragIndex`, `dragOverIndex`) と自動スクロール effect は内部化。ハイライト連動・新規カード末尾スクロールも内蔵。
...
- **app/Ball.tsx** — メインの 3D シーン。キーボード操作、プログラム実行制御、NTAG 書き込み起動、ユーザーガイド (`useGuide` 統合)、デモモード制御を保持。NFC ポーリングは `useNfcScanner` フック、プログラミング UI は `ProgrammingPanel`、設定は `SettingsPanel`、書き込みダイアログは `NtagWriteModal` に委譲。レベルロジックは `useLevel` フックに委譲。`closeProgMode` ヘルパでプログラミングモード終了時の状態リセットを一本化。BRANCH ブロック実行中は if/else サブステップ行とアクティブ分岐ヘッダーをハイライト表示。
...
- **Lv3 モード (条件分岐)**: フッターの Lv3 トグル (F3 キー) で ON/OFF。5x5 グリッド上にランダムなスタート/ゴール (マンハッタン距離 4 以上) + 障害物なし + 1個の「？」セル (条件分岐ポイント、中央3x3内からランダム配置) を配置。外周配置時に発生しうる到着方向依存のデッドロックを回避するため中央3x3に限定。if 文ネストは非対応のため「？」は常に1個。お題なし (`hasChallenge: false`)。代わりに「マップ変更」ボタン (Tab) でマップ再生成。**ゴール到達には「？」セルを一度は経由する必要がある** (`branchUsed` フラグで追跡)。「？」セルは紫ダイヤモンドマーカーで表示され、2つの方向矢印で分岐先を示す。到着方向に基づいて自動的に別方向へ移動: 水平方向 (LEFT/RIGHT) から来た → 垂直方向 (UP or DOWN) へ、垂直方向 (UP/DOWN) から来た → 水平方向 (LEFT or RIGHT) へ。分岐先が壁の場合はバンプ音で停止。7回連続分岐でデッドロック検出 → バースト。`BranchCell` インターフェースで `horizontalBranch` / `verticalBranch` を定義。分岐方向解決は `resolveBranchDir()` に集約。NTAG エンコード: `br` パラメータ (`"12UR"` = col1,row2,hBranch=UP,vBranch=RIGHT)。
...
- **キーボードショートカット**: P=プログラミング ON/OFF、S=設定 ON/OFF、D=2D/3D 切替、F1=Lv1、F2=Lv2、F3=Lv3、Escape=レベル解除 / Playground で 2回押し=デモ開始 (1回押し=デモ停止)、Tab=お題 (Lv1/Lv2) / マップ変更 (Lv3、プログラミングモード時は無効)、Enter=Run (プログラミングモード)、Shift+Enter=New (プログラミングモード)、矢印キー=移動 (プログラミングモード + BRANCH 編集中は if/else 切替)、Space=ジャンプ / つぎへ (Lv1/Lv2 クリア時) / New (プログラミングモード)。言語切替はヘルプページのトップバーボタンのみ (キーボードショートカットなし)。
- **テンキー対応 (BUFFALO BSTKH100)**: NumLock OFF で使用。4/8/6/2=移動 (矢印キー)、5(Clear)=ジャンプ / つぎへ (クリア時) / New (プログラミングモード)、/=プログラミング ON/OFF、*=設定 ON/OFF、9(PageUp)=2D/3D 切替、+=レベルサイクル切替 (OFF→Lv1→Lv2→Lv3→OFF)、-=レベル解除 / Playground で 2回押し=デモ開始、0(Insert)=New、.(Delete)=お題/マップ変更、Tab=お題/マップ変更、Enter=Run (プログラミングモード)。
