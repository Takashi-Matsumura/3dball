...
- **キーボードショートカット**: P=プログラミング ON/OFF、S=設定 ON/OFF、D=2D/3D 切替、J/E/N=言語切替 (日本語/English/Español)、F1=Lv1、F2=Lv2、F3=Lv3、Escape=レベル解除 / Playground で 2回押し=デモ開始 (1回押し=デモ停止)、Tab=お題 (Lv1/Lv2) / マップ変更 (Lv3)、Enter=Run (プログラミングモード)、Shift+Enter=New (プログラミングモード)、矢印キー=移動、Space=ジャンプ / つぎへ (Lv1/Lv2 クリア時) / New (プログラミングモード)。
