...
- **プログラミングモード** — NFC カードで命令列を作成し、Run で順次実行。x2/x3 ループカードで繰り返し、？カードで条件分岐 (if-else)。New / Run ボタンはアイコン付きで低年齢にも直感的。削除ボタン (×) は大きめの正方形タップ領域、カード追加時は最新行まで自動スクロール
...
- ゴールに到達するとファンファーレ + 祝福アニメーション (大きくジャンプしながら 1 回転 + スケールパルス) でクリア。通常の JUMP とは動きが異なる
- **お題カード** (Tab): 目標移動回数を黄色いカードで表示 (「お題 X 回でゴール！」+ 進捗ドット + X/Y カウンタ)。カード内の「更新 [Tab]」で新しいお題、「やめる」でお題なしに戻す
  - 最短経路長 〜 `MAX_CHALLENGE` (15) の範囲でランダム、到達可能な値のみ生成
  - ぴったり達成でクリア、超過・不一致でバーストアニメーション + スタートリセット
...
- **x2/x3 ループカード** (プログラミングモード専用):
  - 直前の命令を繰り返す (`→ ×2` = 右に 2 回)
  - 連続使用は加算 (`→ ×2 ×3` = 右に 5 回)
  - 1 方向あたりの総リピート数は **最大 9** (`MAX_LOOP_REPEAT`) まで。超える X2/X3 は拒否 (バンプ音)
...
| Tab | お題を出す / 更新する (Lv1/Lv2) / マップ変更 (Lv3) |
